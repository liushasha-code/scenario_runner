...
Using BasicEnv to generate a template of testing.
This is a basic template for carla test.

todo: add more function to template

todo: package test module into function outside the general class.
"""
...
from srunner.lyq_code.env import BasicEnv

class TemplateEnv(BasicEnv):
    """
    A carla env template for general test.
    Inherit from BasicEnv.
    """
    ...
    def generate_route(self, start_waypoint, turn_flag=0, hop_resolution=1.0, after_junction_dist=30):
        ...
        # final waypoint
        end_waypoint = middle_waypoint.next(after_junction_dist)[
            0]  # default using first waypoint selection if encounter intersection
...
def main():
    try:
        ...
        origin_transform = carla.Transform(carla.Location(x=5.42, y=120.83, z=1.00),
                                           carla.Rotation(pitch=0.00, yaw=270.00, roll=0.00))

        test = BasicEnv()
        test.draw_waypoint(origin_transform)
        test.set_spectator(origin_transform)
