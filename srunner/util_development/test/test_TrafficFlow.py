...
todo: capsule methods of traffic flow generation.
"""
...
# npc spawn location in Town03 junction scenario
# 30m from junction bounding box
npc_spawn_dict = {
    'left': carla.Transform(carla.Location(x=5.767616, y=175.509048, z=0.000000),
...
class TestTrafficFlow(BasicEnv):
    ...
    def spawn_single_traffic_flow(self):
        """
        A developing version to generate traffic flow.
        """

        # use straight traffic flow for basic design
        transform = npc_spawn_dict['straight']
        transform.location.z = 0.5

        # get simulation time
        if not self.target_spawn_time:
            self.target_spawn_time = 0

        # get a time interval to spawn next vehicle
        # lower_limit = 1.0
        # upper_limit = 5.0
        # time_interval = random.uniform(lower_limit, upper_limit)
        # self.target_spawn_time = self.last_spawn_time + time_interval

        # check spawn condition before spawn vehicle
        # check distance rule satisfied
        if self.straight_npc_list:
            # last_spawned_vehicle =self.straight_npc_list[-1]
            distance = transform.location.distance(self.straight_npc_list[-1].get_transform().location)
        ...
        if now_time >= self.target_spawn_time:
            time_rule = True
        else:
            time_rule = False

        if distance_rule and time_rule:
            # transform = npc_spawn_dict['name']
            # name = key + str(len(count + 1))  # name of the actor to be spawned
            name = str(self.count + 1)

            try:
                vehicle = self.spawn_npc(transform, name)
                self.npc_vehicles.append(vehicle)
                self.straight_npc_list.append(vehicle)
                self.count += 1
                # update last spawn time when new vehicle spawned
                self.last_spawn_time = self.get_time()
                # time interval till spawn next vehicle
                self.target_spawn_time = self.last_spawn_time + self.get_time_interval()
                return vehicle
            except:
