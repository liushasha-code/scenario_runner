...
Not spawn vehicle, using pure vector for visualization.

todo: Unfinished, not very useful
...
def plot_local_coordinate_frame(world, transform, color=0):
    """
    Plot a coordinate frame at a certain transform.
    Notice that the coordinate frame consider only yaw rotation.
    ...
    cy = np.cos(yaw)
    sy = np.sin(yaw)
    x_axis = carla.Location(-sy, cy, 0)

    y_axis = carla.Location(-sy, cy, 0)
    ...
    if color == 0:
        ...
        x_axis_color = carla.Color(r=255, g=20, b=147)  # DeepPink1 255 20 147
        y_axis_color = carla.Color(r=84, g=255, b=159)  # SeaGreen1 84 255 159
        z_axis_color = carla.Color(r=0, g=255, b=255)  # Cyan 0 255 255
    elif color == 2:
        x_axis_color = carla.Color(r=139, g=26, b=26)  # Firebrick4 139 26 26
        y_axis_color = carla.Color(r=152, g=251, b=152)  # PaleGreen 152 251 152
        z_axis_color = carla.Color(r=127, g=255, b=212)  # 	Aquamarine 127 255 212
